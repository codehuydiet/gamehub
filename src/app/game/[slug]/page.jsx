...
import { gameLike, getUser, recentPlayed, getGames } from '@/lib/action';
...
const getData = async (slug) => {
    ...
    if (!res.ok) {
        throw new Error(`Failed to fetch game data for slug: ${slug}`);
    }
    return res.json();
}
...
const SlugGame = async ({ params }) => {
    ...
    const suggest = await getGames()
    console.log(suggest[0].img);


    recentPlayed(slug, user?._id);
    const { name, embed, star, votes, createdAt, updatedAt, technology, platforms } = data;
        ...
        <div className="pl-[0.5rem] pb-[5.25rem] w-[100%] flex justify-center">
            <div className="flex justify-center" id={style.grid_item}>
                <div className="bg-[#ffffff]" id={style.game}>
                    <iframe className='w-[100%] h-[100%]' src={embed} allow="gamepad *;"></iframe>
                </div>
                {user?.id && <LikeComponent user={user} data={data} />}
                {suggest.map((items, index) => {
                    <div key={items.id} className={style.item} style={{ gridArea: `related${index + 1}` }}>
                        <div className={style.warpLink}>
                            <Link className={style.linkContainer} href={items.path}>
                                <div className={style.gameName}>{items.name}</div>
                                <Image className={style.img} src={items.img} fill alt="" />
                            </Link>
                        </div>
                    </div>
                })}
                        ...
                        <div className='pl-[1.5rem] pr-[0.5rem] flex flex-row'>
                            <div className='pr-[1rem] w-[calc(100%-300px)] text-[0.875rem] pt-[1rem] pb-[0.5rem]'>
                                <div className='pb-[0.5rem]'>
                                    <h1 className='text-[1.25rem] leading-[1.167] font-[1000]'>{name}</h1>
                                </div>
                                ...
                                <div className='max-w-[100%] text-[0.875rem] pt-[0.5rem]'>
                                    <div className='flex flex-row'>
                                        ...
                                        <div className='flex'>
                                            <div className='text-[0.875rem] font-[1000] leading-[1.5]'>{star}</div>
                                            <div className='text-[0.75rem] font-[400] flex  leading-[1.5] ml-[0.25rem] pt-[0.1rem]'>({votes} votes)</div>
                                        </div>
                                    </div>
                                    <div className='flex flex-row'>
                                ...
                                <div className='mb-[0.875rem]'>
                                    <p>{name} is an game set in a top-down online battle arena. Collect orbs and battle other players to evolve your warrior. Each evolution improves your weapon range but slows your movement. How many will you slay before you fall?</p>
                                </div>
                                <div className=''>
